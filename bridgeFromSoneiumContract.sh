...
REQUIRED_VARS=(
  ...
  SONEIUM_REBASE_TOKEN_ADDRESS
  SONEIUM_LINK_ADDRESS
  SONEIUM_ROUTER
  SONEIUM_BRIDGE_CONTRACT
  BASE_CHAIN_SELECTOR
...
function call_bridge() {
  ...
  echo "🔁 Approving ${amount} of ${SONEIUM_REBASE_TOKEN_ADDRESS} for Bridge contract..."
  cast send "${SONEIUM_REBASE_TOKEN_ADDRESS}" \
  ...
  cast send "${SONEIUM_LINK_ADDRESS}" \
    "approve(address,uint256)" \
    "${SONEIUM_BRIDGE_CONTRACT}" \
    "5000000000000000000" \
    --rpc-url "${SONEIUM_MINATO_RPC_URL}" \
  ...
  echo "🔁 Bridging ${amount} from Soneium → ${destName}..."
  cast send "${SONEIUM_BRIDGE_CONTRACT}" \
